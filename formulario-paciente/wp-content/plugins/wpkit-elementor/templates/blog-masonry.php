...
$blog_section_id = 'wke-masonry-blog-' . WKE_Util::random_string( 10 );

$renderHTML='<div id="' . esc_attr( $blog_section_id ) . '" class="wke-masonry-blog ' . esc_html( $columns ) . ' ' . esc_html( $extra_class ) . '">';
                ...
                while ( $wke_blog_query->have_posts() ) : $wke_blog_query->the_post();
                   ...
                   $renderHTML .= '<div class="wke-post post-' . esc_attr( get_the_ID() ) . '">';
                     ...
                     if( has_post_thumbnail() ){
                        $renderHTML .= '<div class="wke-post-thumbnail">
                          <a href=" ' . esc_url( get_permalink() ) . ' "><img src="' . get_the_post_thumbnail_url( get_the_ID(), 'full' ) . '" /></a>
                        </div>';
                     }
                     ...
                     $renderHTML .= '<div class="wke-post-entry">';
                         ...
                         //Post Excerpt
                         $renderHTML .= '<p class="wke-post-excerpt">' . WKE_Util::truncate_string( esc_html( get_the_excerpt() ), 200 ) . '</p>';
